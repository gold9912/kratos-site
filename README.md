...
Для продакшена нужен настоящий Supabase проект и заполненные переменные `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`. Без них backend запускается, но endpoints, завязанные на базу и Auth, будут возвращать ошибку конфигурации.
