...
- `admin` — защищенное редактирование параметров калькулятора.

Публичный каталог услуг и калькулятор используют одну таблицу `services`, без дополнительной схемы. Позиции калькулятора отличаются категорией `calculator:<раздел>`, поэтому клиент показывает в калькуляторе только измеряемые работы (`м²`/`шт`) и группирует их по разделам.
